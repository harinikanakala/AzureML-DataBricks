...
table_path = "dbfs:/nagadhara.kanakala@teamhgs.com/delta/wine_data"
...
# MAGIC ## Model serving
# MAGIC
# MAGIC To productionize the model for low latency predictions, use Databricks Model Serving ([AWS](https://docs.databricks.com/machine-learning/model-serving/index.html)|[Azure](https://docs.microsoft.com/azure/databricks/machine-learning/model-serving/index)) to deploy the model to an endpoint.
# MAGIC
...
def score_model(dataset):
  url = 'https://adb-4882752041302442.2.azuredatabricks.net/model/wine_quality/5/invocations'
  headers = {'Authorization': f'Bearer {os.environ.get("DATABRICKS_TOKEN")}', 'Content-Type': 'application/json'}
  input_data = X_test_df[:num_predictions].to_dict(orient='records')
  #print(input_data)
...
inputdata = { "signature_name": "test", "instances": 1, "inputs": 1}
# inputdata = {'fixed_acidity':1, 'volatile_acidity':2, 'citric_acid', 'residual_sugar', 'chlorides', 'free_sulfur_dioxide', 'total_sulfur_dioxide', 'density', 'pH', 'sulphates', 'alcohol', 'is_red'
served_predictions = score_model(input_data)
print(served_predictions)
#print(X_test[:num_predictions])
...
print(X_test_df.head(0))
# Convert the input features to a list of dictionaries
input_data = X_test_df[:num_predictions].to_dict(orient='records')
print(input_data)
inputdata = { "signature_name": "test", "instances": 1, "inputs": 1}

# Call the score_model function with the correct input data format
#served_predictions = score_model(create_tf_serving_json(input_data))
served_predictions = score_model(input_data)
# Compare the results from the deployed model and the trained model
model_evaluations = model.predict(X_test[:num_predictions].values.tolist())
pd.DataFrame({
    "Model Prediction": model_evaluations,
    "Served Model Prediction": served_predictions,
})
...
# COMMAND ----------

 {
  "dataframe_split": {
    "index": [0,1],
    "columns": ["fixed_acidity","volatile_acidity","citric_acid","residual_sugar","chlorides","free_sulfur_dioxide","total_sulfur_dioxide","density","pH","sulphates","alcohol","is_red"],
    "data": [[7.6,0.31,0.29,10.5,0.04,21.0, 145.0,0.9966,3.04,0.35,9.4,0],[7.6,0.31,0.29,10.5,0.04,21.0,145.0,0.9966,3.04, 0.35,9.4,0]]
}
  }
